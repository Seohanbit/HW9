...
{
    if (head == NULL) {
		printf("\n Nothing to delete!!\n");             //삭제할 노드가 아무것도 없으면 없다고 출력하는 전처리기
		return -1;
	}
	...
	Node* ptr = head->left;                             //ptr을 head의 left가 가리키는 곳이 되게 함
	Node* parentNode = head;                            //parentNode를 head로 초기화 함

	while(ptr != NULL) {

		if(ptr->key == key) {
			if(ptr->left == NULL && ptr->right == NULL) {       //찾는 key값과 node의 key값이 같더라도 left와 right에 자식이 있다면 삭제할 수 없음

				/* root node case */
				if(parentNode == head)                          //부모노드가 head와 같다면 head의 left가 NULL값을 가리키게 하고 ptr을 메모리 해제
					head->left = NULL;

				/* left node case or right case*/
				if(parentNode->left == ptr)                     //parentNode의 left가 ptr과 같으면
					parentNode->left = NULL;                    //부모노드의 left를 NULL로 초기화
				else                                            //right가 ptr과 같으면
					parentNode->right = NULL;                   //right를 NULL로 초기화

				free(ptr);                                      //ptr 메모리 해제
			}
			else {
				printf("the node [%d] is not a leaf \n", ptr->key); //삭제하려는 node가 자식 node를 가지고 있을경우 leaf가 아니라는 경고문 출력
			}
			return 1;
		}
		...
		if(ptr->key < key)                                      //ptr->key가 받아온 key보다 작으면 ptr을 ptr->right로 크면 ptr->left로 초기화
			ptr = ptr->right;
		else
			ptr = ptr->left;
	...
	printf("Cannot find the node for key [%d]\n ", key);		//받아온 key값과 같은 값을 가진 node는 없다고 출력
...
{
    if(ptr == NULL)                                 //ptr이 NULL이면 NULL값 리턴
		return NULL;

	if(ptr->key < key)
		ptr = searchRecursive(ptr->right, key);     //ptr의 key값이 받아온 key값보다 작을 경우 ptr의 right를 함수의 인자로 반환
	else if(ptr->key > key)
		ptr = searchRecursive(ptr->left, key);      //ptr의 key값이 받아온 key값보다 클 경우 ptr의 left를 함수의 인자로 반환

	/* if ptr->key == key */
	return ptr;                                     //ptr의 key값과 받아온 key값이 같을 경우 ptr을 리턴함
}

Node* searchIterative(Node* head, int key)          //iterative 방식으로 검색
{
       /* root node */
	Node* ptr = head->left;                         //ptr이 head의 left가 가리키는 곳이 되게 함

	while(ptr != NULL)
	{
		if(ptr->key == key)                         //ptr의 key값이 받아온 key값과 같으면 ptr을 리턴
			return ptr;

		if(ptr->key < key) ptr = ptr->right;        //ptr의 key값이 받아온 key값보다 작으면 ptr을 ptr의 right가 가리키는 곳이 되게 함
		else
			ptr = ptr->left;                        //받아온 key값보다 크면 ptr을 ptr의 left가 가리키는 곳이 되게 함
	}
